function [S,L,M,diagnostic] = solver_RPCA_BOT_Beckman_ADMM(imsize,Y,Phi,lambda,gamma,kappa,opts)
% solver_RPCA_BOT_Beckman_ADMM
% 
% This solves the RPCA with balanced optimal transport regularization
% problem via ADMM. To make the optimal transport regularization tractable,
% we utilized Beckman's formulation.
%
% [S,L,diagnostic] = solver_RPCA_BOT_Beckman_ADMM(A, b, lambda, rho, alpha)
% solves the following problem via ADMM:
%
%   min sum_t { 1/2*|| Phi(:,:,t)*(S(:,t)+L(:,t)) - Y(:,t) ||_2^2 }
%       + lambda*|| vec(S) ||_1
%       + gamma*|| L ||_*
%       + kappa*sum_{t=1}^{T-1} W( S(:,t) , S(:,t+1) )
%   subject to
%       S,L >= 0
%       
...
for k = 1:maxiter
    ...
    % solve for Z and W
    [ZW,M,D] = ...
        Prox_Beckman([S(:,idx1)-B;S(:,idx2)-C],rho/kappa,Div,ZW,M,D,beckopts);
...
% K_op = @(m,x,r) real(conj(Div)*m) + A_op(x);
% disp(['Equality constrain error = ' num2str( norm(K_op(M,ZW),'fro') )]);
...
function [x,m,d] = Prox_Beckman(y,rho,D_op,x,m,d,opts)
tau1    = opts.tau1;
tau2    = opts.tau2;
maxiter = opts.maxiter;
A_op = @(x) x(1:end/2,:) - x(end/2+1:end,:);
At_op = @(z) [z;-z];
K_op = @(m,x,r) real(conj(D_op)*m) + A_op(x);
K_op_mx = K_op(m,x);
for k = 1:maxiter
