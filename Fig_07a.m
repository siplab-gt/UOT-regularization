...
% Search parameters
% redo_search = [0,1,0,0]; load([filename '.mat']); % comment if full search required
...
for i = 1:length(noise_sigma_space)
    % Update trial variable
    sim_param.noise_sigma = noise_sigma_space(i);
...
%               'L_rMSE = ' num2str( L_rMSE(s,t) ) ]);   
    ...
    ts_disp(['Summary statistics of ' num2str(size(results,3)) ' trials '...
             'for ' var_name ' = ' num2str(sim_param.noise_sigma) ]);
    ...
    % Display Progress
    ts_disp(['Progress of ' filename ' : ' num2str(i/length(noise_sigma_space)*100) '% completed.']);
end
...
% Compute metrics
S_rMSE  = nan(length(solvers),length(noise_sigma_space),nbr_trials);
L_rMSE  = nan(length(solvers),length(noise_sigma_space),nbr_trials);
F1      = nan(length(solvers),length(noise_sigma_space),nbr_trials);
runtime = nan(length(solvers),length(noise_sigma_space),nbr_trials);
for i = 1:length(noise_sigma_space)
    sim_param.noise_sigma = noise_sigma_space(i); % Update trial variable
    for t = 1:nbr_trials
    for s = 1:length(solvers)
        % Simulate Problem
        rng(t,'twister'); [~,~,S_gt,L_gt] = simulator(sim_param);
...
for s = 1:length(solvers)
    subplot(411);
%     h(1,s).mainLine = errorbar(noise_sigma_space,mean(squeeze(rMSE(s,:,:))'),std(squeeze(rMSE(s,:,:))'),...
%                                'LineWidth',2,'Color',cmap(s,:),'Marker',markers{s});
    h(1,s) = shadedErrorBar(noise_sigma_space,squeeze(S_rMSE(s,:,:))',{@nanmean,@nanstd},...
...
%     h(1,s).mainLine = errorbar(noise_sigma_space,mean(squeeze(rMSE(s,:,:))'),std(squeeze(rMSE(s,:,:))'),...
%                                'LineWidth',2,'Color',cmap(s,:),'Marker',markers{s});
    h(2,s) = shadedErrorBar(noise_sigma_space,squeeze(L_rMSE(s,:,:))',{@nanmean,@nanstd},...
                            'lineprops',{['-' markers{s}],'LineWidth',2,'Color',cmap(s,:),'MarkerFaceColor',cmap(s,:)}); hold on;
    L_rMSE_lines = [L_rMSE_lines, h(2,s).mainLine];
    hold on;
    
    subplot(413);
%     h(2,s).mainLine = errorbar(noise_sigma_space,mean(squeeze(F1(s,:,:))'),std(squeeze(F1(s,:,:))'),...
%                                'LineWidth',2,'Color',cmap(s,:),'Marker',markers{s});
    h(3,s) = shadedErrorBar(noise_sigma_space,squeeze(F1(s,:,:))',{@nanmean,@nanstd},...
                            'lineprops',{['-' markers{s}],'LineWidth',2,'Color',cmap(s,:),'MarkerFaceColor',cmap(s,:)}); hold on;
    F1_lines = [F1_lines, h(3,s).mainLine];
    hold on;
    
    subplot(414);
%     h(2,s).mainLine = errorbar(noise_sigma_space,mean(squeeze(runtime(s,:,:))'),std(squeeze(F1(s,:,:))'),...
%                                'LineWidth',2,'Color',cmap(s,:),'Marker',markers{s});
    h(3,s) = shadedErrorBar(noise_sigma_space,squeeze(runtime(s,:,:))',{@nanmean,@nanstd},...
                            'lineprops',{['-' markers{s}],'LineWidth',2,'Color',cmap(s,:),'MarkerFaceColor',cmap(s,:)}); hold on;
...
subplot(411);
% loglog(noise_sigma_space,mean(rMSE,3)','LineWidth',2);
...
% legend(S_rMSE_lines,solvers,'FontSize',fontSize,'Location','NorthWest');
...
subplot(413); 
%loglog(noise_sigma_space,mean(F1,3)','LineWidth',2);
axis tight; grid on; grid minor; ylim([0,1]);
set(gca, 'XScale', 'log'); 
% xlabel(var_name,'FontSize',fontSize);
...
subplot(414); 
%loglog(noise_sigma_space,mean(runtime,3)','LineWidth',2);
axis tight; grid on; grid minor; %ylim([0,1]);
set(gca, 'XScale', 'log'); 
xlabel(var_name,'FontSize',fontSize);
