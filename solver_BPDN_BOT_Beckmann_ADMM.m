function [s,m,diagnostic] = solver_BPDN_BOT_Beckmann_ADMM(imsize,y,Phi,x0,lambda,kappa,opts)
% solver_BPDN_BOT_Beckman_ADMM
% 
% This solves BPDN problem with balanced optimal transport regularization
% via ADMM. To make the optimal transport regularization tractable,
% we utilize Beckmann's formulation.
%
%   min 1/2 * || y - Phi*s ||_2^2
%       + lambda * || s ||_1
%       + kappa  * W( s , s0 ) )
%   subject to
%       s >= 0
...
for k = 1:maxiter
    ...
    % solve for s (L1 regularization)
    s = Prox_NonNeg_l1((x + a + z + b)/2, lambda/rho/2 );
